...
export default function timePlan(lists = {}, action) {
  ...
  switch (action.type) {
    ...
    case types.MERGE_TAG:
      if (action.direction == 'up' || action.direction == 'all'){
        for (let i = 0; i < newLists.length; i ++){
          if (newLists[i].startIndex == action.startIndex){ //只需要找到开始或结束就可以确定这个tag，因为都是已经处理好
            if (i > 0 && typeof(newLists[i - 1].tagName) != 'undefined' && newLists[i - 1].tagId == action.tagId) { //第1个元素没有上一个 //是tag才可能合并
              newLists.splice(i - 1, 2);
              newLists.push({startIndex:newLists[i - 1].startIndex, endIndex:action.endIndex, tagId:action.tagId, tagName:action.tagName});
            }
            break;
          }
        }
      ...
      if (action.direction == 'down' || action.direction == 'all'){
        for (let i = 0; i < newLists.length; i ++){
          if (newLists[i].startIndex == action.startIndex){ //只需要找到开始或结束就可以确定这个tag，因为都是已经处理好
            if (i < newLists.length - 1 && typeof(newLists[i + 1].tagName) != 'undefined' && newLists[i + 1].tagId == action.tagId) { //最后一个元素没有下一个 //是tag才可能合并
              newLists.splice(i, 2);
              newLists.push({startIndex:action.startIndex, endIndex:newLists[i + 1].endIndex, tagId:action.tagId, tagName:action.tagName});
            }
            break;
          }
        }
