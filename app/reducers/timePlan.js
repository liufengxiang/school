...
export default function timePlan(lists = {}, action) {
  ...
  switch (action.type) {
    ...
    case types.DEL_TAG:
      ...
      newLists = [...lists[weekId]]; //如果直接赋值 newLists = lists 则是传引用，不能
    ...
    case types.CLEAR_TIME_PLAN_LIST:
      return {};
    case types.MERGE_TAG:
      newLists = [...lists[weekId]];
      let mergeUp = false;
      let mergeDown = false;
      let upStartIndex = 0;
      let downEndIndex = 0;
      for (let i = 0; i < newLists.length; i ++){
        if (newLists[i].startIndex == action.startIndex){ //只需要找到开始或结束就可以确定这个tag，因为都是已经处理好
          //判断是否可以向上合并
          if (i > 0 && typeof(newLists[i - 1].tagName) != 'undefined' && newLists[i - 1].tagId == action.tagId) { //第1个元素没有上一个 //是tag才可能合并
            //重要,splice删除操作后,下面不能再用startIndex:newLists[i - 1].startIndex,因为数组数量已经变化,要将原来startIndex:newLists[i - 1].startIndex的值先存起来再删除
            mergeUp = true;
            upStartIndex = newLists[i - 1].startIndex; //合并开始下标为上一个元素的开始下标
          }
          //判断是否可以向下合并
          if (i < newLists.length - 1 && typeof(newLists[i + 1].tagName) != 'undefined' && newLists[i + 1].tagId == action.tagId) { //最后一个元素没有下一个 //是tag才可能合并
            mergeDown = true;
            downEndIndex = newLists[i + 1].endIndex;
          }
          //合并
          if (mergeUp && mergeDown) {
            newLists.splice(i - 1, 3);
            newLists.push({startIndex:upStartIndex, endIndex:downEndIndex, tagId:action.tagId, tagName:action.tagName});
          }else if (mergeUp && !mergeDown){
            newLists.splice(i - 1, 2);
            newLists.push({startIndex:upStartIndex, endIndex:action.endIndex, tagId:action.tagId, tagName:action.tagName});
          }else if (!mergeUp && mergeDown){
            newLists.splice(i, 2);
            newLists.push({startIndex:action.startIndex, endIndex:downEndIndex, tagId:action.tagId, tagName:action.tagName});
          }else {
            //不合并
          }
          break;
